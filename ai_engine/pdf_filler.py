...
class OrsiniPDFFiller:
    ...
    def _fill_page_1(self, page: fitz.Page, d: dict[str, Any]) -> None:
        ...
        # Checkboxes (draw neat X crosses ONLY if explicitly True)
        if d.get("standard_precautions_maintained") is True:
            self._draw_cross(page, (32, 307))
        if d.get("lungs_clear") is True or "clear" in str(d.get("lung_sounds", "")).lower():
            self._draw_cross(page, (79, 550))
        if d.get("heart_sounds_normal") is True:
            self._draw_cross(page, (279, 353))
        if d.get("heart_sounds_regular") is True or "regular" in str(d.get("heart_sounds", "")).lower():
            self._draw_cross(page, (317, 353))
        if d.get("chest_pain_denies") is True:
            self._draw_cross(page, (230, 420))
        if d.get("alert") is True:
            self._draw_cross(page, (230, 560))
        if d.get("oriented_to_person") is True:
            self._draw_cross(page, (260, 560))
        if d.get("oriented_to_place") is True:
            self._draw_cross(page, (334, 560))
        if d.get("oriented_to_time") is True:
            self._draw_cross(page, (365, 560))
        if d.get("pupils_both") is True:
            self._draw_cross(page, (340, 600))
        if d.get("skin_no_deficit") is True:
            self._draw_cross(page, (447, 353))
        if d.get("bowel_sounds_active") is True or "active" in str(d.get("bowel_sounds", "")).lower():
            self._draw_cross(page, (447, 423))
        if d.get("fall_precaution_maintained") is True:
    ...
    def _fill_page_2(self, page: fitz.Page, d: dict[str, Any]) -> None:
        ...
        # Page 2 Checkboxes (exact box-centered coordinates - ONLY if explicitly specified)
        if d.get("type_of_access"):
            if "peripheral" in str(d.get("type_of_access")).lower() or "piv" in str(d.get("type_of_access")).lower():
        ...
        if d.get("site_condition_dry") or "dry" in site_note.lower():
            self._draw_cross(page, (80.5, 264.6))

        if d.get("current_dressing_intact") is True or "intact" in str(d.get("current_dressing_intact", "")).lower():
            self._draw_cross(page, (109.7, 277.7))

        if d.get("dressing_cleansed_chloraprep"):
            self._draw_cross(page, (319.5, 290.8))

        if d.get("dressing_type_transparent"):
            self._draw_cross(page, (429.7, 290.8))

        if d.get("piv_inserted_this_visit") is True:
            self._draw_cross(page, (305.4, 304.3))

        if d.get("catheter_discontinued_this_visit") is True:
            self._draw_cross(page, (177.3, 330.3))

        if d.get("therapy_completed") is True:
            self._draw_cross(page, (28.6, 356.9))

        if d.get("pump_program_verified") is True or str(d.get("pump_program_verified", "")).lower() in ("yes", "true"):
            self._draw_cross(page, (362.8, 451.6))

        if d.get("pump_settings_verified_with_label") is True or str(d.get("pump_settings_verified_with_label", "")).lower() in ("yes", "true"):
            self._draw_cross(page, (180.2, 465.0))

        if d.get("pump_settings_changed") is False or str(d.get("pump_settings_changed", "")).lower() in ("no", "false"):
            self._draw_cross(page, (390.4, 465.0))

        if d.get("therapy_admin_by") and "nurse" in str(d.get("therapy_admin_by")).lower():
            self._draw_cross(page, (90.1, 543.0))

        if d.get("labs_drawn") is False:
    ...
    def _fill_page_3(self, page: fitz.Page, d: dict[str, Any]) -> None:
        ...
        nurse = str(d.get("clinician_name_title") or "").strip()
        if not nurse and sig:
            nurse = sig if "BSN" in sig else f"{sig}, BSN"
    ...
    def _fill_page_4(self, page: fitz.Page, d: dict[str, Any]) -> None:
        ...
        if d.get("instructed") is True:
            self._draw_cross(page, (136.4, 233.6))  # Instructed
        if d.get("pt_cg_verbalized_understanding") is True:
